...
for (f in c(EI_perRegionPerSample_files)) {
  ...
  EI_perRegionPerSample_pooled = EI_perRegionPerSample %>%
    ...
    summarise(across(c(Length:AntisenseGeneCommonName, TotalSamplesPerGroup), first),
              across(matches("NumOf[ACGT]2[ACGT]Mismatches|NumOf[ACGT]|TotalCoverageAtAllPositions"), sum),
              SampleCount = n()) %>%
    mutate(MeanCoveragePerRegion = TotalCoverageAtAllPositions / Length,
           
           A2GEditingIndex = 100 * NumOfA2GMismatches /  (NumOfA2GMismatches + NumOfA),
           A2CEditingIndex = 100 * NumOfA2CMismatches /  (NumOfA2CMismatches + NumOfA),
           A2TEditingIndex = 100 * NumOfA2TMismatches /  (NumOfA2TMismatches + NumOfA),
           
           C2AEditingIndex = 100 * NumOfC2AMismatches /  (NumOfC2AMismatches + NumOfC),
           C2GEditingIndex = 100 * NumOfC2GMismatches /  (NumOfC2GMismatches + NumOfC),
           C2TEditingIndex = 100 * NumOfC2TMismatches /  (NumOfC2TMismatches + NumOfC),
           
           G2AEditingIndex = 100 * NumOfG2AMismatches /  (NumOfG2AMismatches + NumOfG),
           G2CEditingIndex = 100 * NumOfG2CMismatches /  (NumOfG2CMismatches + NumOfG),
           G2TEditingIndex = 100 * NumOfG2TMismatches /  (NumOfG2TMismatches + NumOfG),
           
           T2AEditingIndex = 100 * NumOfT2AMismatches /  (NumOfT2AMismatches + NumOfT),
           T2CEditingIndex = 100 * NumOfT2CMismatches /  (NumOfT2CMismatches + NumOfT),
           T2GEditingIndex = 100 * NumOfT2GMismatches /  (NumOfT2GMismatches + NumOfT),
  ...
  EI_perRegionPerSample_pooled_count = EI_perRegionPerSample %>%
    left_join(EI_perRegionPerSample_pooled %>%
                 mutate(A2G = if_else(A2GEditingIndex > T2CEditingIndex, "+", 
                                      if_else(A2GEditingIndex < T2CEditingIndex, "-", "unknown"))) %>%
                 select(AluElement, A2G)) %>%
    # lazy_dt() %>%
    group_by(across(all_of(group_by_vars))) %>%
    # count number of samples
    summarise(SampleCount = n(),
              # at least A2G MM = edited sample
              SampleCountEdited = if_else(A2G == "+", sum(NumOfA2GMismatches >= 0, na.rm = T),
                                          if_else(A2G == "-", sum(NumOfT2CMismatches > 0, na.rm = T),
                                                as.integer(0)))) %>%
