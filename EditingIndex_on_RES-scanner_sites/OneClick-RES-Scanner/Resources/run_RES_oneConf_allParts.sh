...
parser.add_argument('-b', '--bwa_dir',type=str, help='Path of BWA ', required=False, default='/private/common/Software/BWA/bwa-0.6.2/bwa')
parser.add_argument('-s', '--samtools_dir',type=str, help='Path of samtools ', required=False, default='/private/apps/bin/samtools')
parser.add_argument('-l', '--blat_dir',type=str, help='Path of blat ', required=False, default='/private/apps/bin/blat')
EOF
...
perl ${res_S_DIR}/RES-Scanner_alignment/RES-Scanner_alignment.part1.pl --ref $REF_FILE --outDir $OUT_DIR --bwa $BWA_DIR --config $CONF_FILE 

sh "/home/alu/twerski/Scripts/BASH/RES_STEPS/run_step_0_1.sh" $OUT_DIR 

perl ${res_S_DIR}/RES-Scanner_alignment/RES-Scanner_alignment.part2.pl --config $CONF_FILE --ref $REF_FILE --outDir $OUT_DIR -t 12 --bwa $BWA_DIR --samtools $SAMTOOLS_DIR --index 1 
...
perl ${res_S_DIR}/RES-Scanner_identification/RES-Scanner_identification.pl --config ${OUT_DIR}"RES_Scanner_indentification_config.txt" --out $IDENTIFICATION_OUT_DIR --genome $REF_FILE --blat $BLAT_DIR --samtools $SAMTOOLS_DIR $ADDITIONAL_ARGUMENTS
